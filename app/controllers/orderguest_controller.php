...
class OrderGuestController extends BaseController{
    private $orderModel;
    ...
    public function index($page){
        $limit = 10; 
        $start = ($page - 1) * $limit;
        $totalOrders = $this->guestModel->getTotalOrdersGuest();
        $listOrders = $this->guestModel->getListOrdersGuest($start, $limit);
        $totalPages = ceil($totalOrders/$limit);
        $data = array('listOrders' => $listOrders, 'totalPages' => $totalPages);
        $this->renderAdmin('orders_guest', $data);
    }
    ...
    public function detailOrder(){
        if(isset($_GET['order_id'])){
            ...
            $this->renderAdmin('detail_orderguest', $data);
        }
    }
    public function search()
    {
        ...
        $this->renderAdmin('search_orderguest', $data);
    }
